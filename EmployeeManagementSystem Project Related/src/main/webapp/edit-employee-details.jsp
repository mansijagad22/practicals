	...
	<div class="container">
		<div class="form-wrap">
			<form action="ProfileServletForEditEmp" method="post">
				...
				<div class="form-group">
					<label for="hobbies">Hobbies : </label> <input type="text" name="hobbies"
						value="<%= emp.getHobbies() %>" class="input" />
				</div>
				<div class="form-group">
					<label for="admin" class="admin-label"> Admin : </label> <input
						class="radioBtn" type="radio" 
						<%if(emp.getIsAdmin()==1){ %>id="yes" name="fav_language" 
						value="<%= emp.getIsAdmin()  %>">
						<%
						}
						%>
						<% if(emp.getIsAdmin()==0) { %>
					...
					<input class="radioBtn" type="radio" id="no" name="fav_language"
						value="<%= emp.getIsAdmin()  %>"> <label class="radiobtn-Label" for="no">No</label>
<%} %>
				</div>
				<%
				...
				<div id="repeater">
					...
					<div class="items" data-group="test">
						...
						<div class="form-group">
								...
								<div class="col-lg-10">
								<input type = "text" class = "form-control" id="inputStreet" name ="street[<%=i%>]"
								value="<%= street.get(i) %>" data-name = "street"/>
								</div>
							</div>
							<div class="form-group">
								<label for="inputEmail" class="col-lg-2 control-label">City</label>
								<div class="col-lg-10">
									<input type="text" class="form-control" id="inputCity" name= "city[<%=i%>]"
									value="<%=city.get(i)%>"
										data-name="city">
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-lg-10">
									<input type="text" class="form-control" id="inputState" name = "state[<%=i%>]"
										value="<%=state.get(i)%>"
										data-name="state">
								</div>
							</div>
							<div class="form-group">
								<label for="inputEmail" class="col-lg-2 control-label">Pin
									code</label>
								<div class="col-lg-10">
									<input type="text" class="form-control" id="inputPinCode" name = "pinCode[<%=i%>]"
									value="<%=pinCode.get(i) %>"
										data-name="pincode">
								</div>
							</div>
						...
						<div class="pull-right repeater-remove-btn">
							<input type="button" value = "Remove" class="btn btn-danger remove-btn">
						</div>
						<div class="clearfix"></div>
					</div>
