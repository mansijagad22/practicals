...
//			List<Project> newProjectList = new ArrayList<Project>();
		try {
			PreparedStatement ps = con.prepareStatement("select * from employee where is_admin=0");
...
//				String name = rs.getString("project_name");
	...
	public int updateEmpDetails(Employee e) {
		int rowUpdated = 0;
...
//					+ " emp_id = ?");
			...
			ps = con.prepareStatement(
					"UPDATE employee SET emp_name=? ,mob_no = ?,dob= ?,hobbies = ?,is_admin = ?,email = ? ,password = ? where emp_id = ?");
	...
	public int selectLatestEmpId() {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("select Max(emp_id) as emp_id from employee order by emp_id desc");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
//				String name = rs.getString("project_name");
				m = rs.getInt("emp_id");
	...
	// to delete any employee detail by its id
	public void deleteById(int id) {
		boolean rowDeleteFromAddress = false;
		boolean rowUpdateFromEmployee = false;

		try {

			PreparedStatement ps1 = con.prepareStatement("delete from addresses where emp_id = ?");
			PreparedStatement ps = con.prepareStatement("delete from employee where emp_id = ?");
			ps1.setInt(1, id);
			ps.setInt(1, id);
			rowDeleteFromAddress = ps.executeUpdate() > 0;
			rowUpdateFromEmployee = ps1.executeUpdate() > 0;
	...
	public List<Employee> selectAllEmpByName(String s) {
		List<Employee> empList = new ArrayList<>();
		try {
			PreparedStatement ps = con.prepareStatement("select * from employee where emp_name LIKE ?");
			System.out.println(ps + " end");
			ps.setString(1, "%" + s + "%");
//						     statement.setString(1, "%" + username + "%");
		...
		System.out.println("last line on empDao " + empList);
		return empList;
	}
